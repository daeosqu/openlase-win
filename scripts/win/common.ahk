...
;; OpenLase startup utility - Common include file for AHK
...
RunSimulator(arguments="", runparams="", activate=False, hide_console=False, minimize_console=True)
{
...
RunOutput(arguments="", runparams="", activate=False, hide_console=False, minimize_console=True)
{
  cmd_name := "output.exe"
  cmd := cmd_name . " " . arguments
...
RunQjackCtl(arguments="", runparams="", activate=False)
{
...
ActivateOrStartProgram(windowTitle, programName, activate=False, options="", console_title="", minimize_console=True, hide_console=False)
{
  ...
  If !hwnd
  {
    ...
    hwnd := ActivateWindow(windowTitle, wait_invoke)
  } else if activate {
    hwnd := ActivateWindow(windowTitle, wait_exists)
  }
